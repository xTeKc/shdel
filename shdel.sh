...
# run shdel. It just does platform detection, downloads the binary
# and runs it.
...
if [ "$KSH_VERSION" = 'Version JM 93t+ 2010-03-05' ]; then
    ...
    echo 'shdel does not work with this ksh93 version; please try bash!' >&2
    exit 1
fi
...
main() {
    ...
    if $_ansi_escapes_are_valid; then
        printf "\33[1m\33[32minfo:\33[0m downloading shdel\n" 1>&2
    else
        printf '%s\n' 'info: downloading shdel' 1>&2
    fi
    ...
    if [ ! -x "$_file" ]; then
        ...
        printf '%s\n' "Please copy the file to a location where you can execute binaries and run ./shdel${_ext}." 1>&2
        exit 1
    fi
